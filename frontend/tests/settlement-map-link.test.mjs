...
test('admin exposes manual settlement boundary editor with polygon and radius modes', async () => {
  ...
  assert.match(editorSource, /Посчитать полностью внутри/)
  assert.match(editorSource, /Импортировать участки NSPD полностью внутри/)
  assert.match(editorSource, /не полностью внутри/)
